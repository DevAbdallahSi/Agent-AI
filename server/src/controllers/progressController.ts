...
export const addXP = async (req: AuthRequest, res: Response, next: NextFunction): Promise<Response | void> => {
    try {
        ...
        return res.json({  // return added here
            message: `Added ${amount} XP`,
            totalXP: user.xp,
            level: user.level
        })
    } catch (error) {
        return next(error)  // return added here
    }
}

export const awardBadge = async (req: AuthRequest, res: Response, next: NextFunction): Promise<Response | void> => {
    try {
